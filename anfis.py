...
class AntecedentLayer(torch.nn.Module):
    ...
    def __init__(self, varlist):
        ...
        self.mf_indices = torch.tensor(list(mf_indices)).to(device)
...
class ConsequentLayer(torch.nn.Module):
    ...
    def __init__(self, d_in, d_rule, d_out):
        ...
        self._coeff = torch.zeros(c_shape, dtype=dtype, requires_grad=True).to(device)
    ...
    def fit_coeff(self, x, weights, y_actual):
        ...
        x_plus = torch.cat([x, torch.ones(x.shape[0], 1).to(device)], dim=1)
...
class AnfisNet(torch.nn.Module):
    ...
    def __init__(self, description, invardefs, outvarnames, hybrid=True):
        ...
        mfdefs = [FuzzifyVariable(mfs).to(device) for _, mfs in invardefs]
        self.num_in = len(invardefs)
        self.num_rules = np.prod([len(mfs) for _, mfs in invardefs])
        if self.hybrid:
            cl = ConsequentLayer(self.num_in, self.num_rules, self.num_out).to(device)
        else:
            cl = PlainConsequentLayer(self.num_in, self.num_rules, self.num_out).to(device)
        self.layer = torch.nn.ModuleDict(OrderedDict([
            ('fuzzify', FuzzifyLayer(mfdefs, varnames).to(device)),
            ('rules', AntecedentLayer(mfdefs).to(device)),
    ...
    def forward(self, x):
        ...
        self.weights = self.raw_weights / (sum_weights + 1e-4)
