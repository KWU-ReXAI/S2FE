...
sweep_config = {
	"method": "bayes",  # grid, random, bayes 중 택1
	"metric": {
		"name": "CAGR",
		"goal": "maximize"
	},
...
def sweep():
	with wandb.init() as run:
		...
		subprocess.run(f"python train.py --testNum 5 --lr_MLP {config.lr_MLP} --lr_anfis {config.lr_anfis} --epochs_MLP {config.epochs_MLP} --epochs_anfis {config.epochs_anfis} --hidden {config.hidden} --Validation", shell=True)
...
sweep_id = wandb.sweep(sweep_config, project="s3ce_hyper_tuning_final")
