...
for cluster_index in range(3):
    ...
    for sector in sector_list:
        ...
        df_sector_stocks.to_csv(f"./data_kr/verification/cluster_init/cluster_{cluster_index}_init.csv", encoding='utf-8-sig')


    # 결측치 계산
    nan_ratio = df_sector_stocks.isna().mean()
    over_50_columns = nan_ratio[nan_ratio>=0.5].index.to_list()
    print(f'클러스터 번호: ', cluster_index , '\n결측치:\n', nan_ratio, '\n0.5 넘는 결측치: ', over_50_columns, '\n')
    nan_ratio.to_csv(f"./data_kr/verification/cluster_NaN/cluster_{cluster_index}_NaN.csv", encoding='utf-8-sig')
    ...
    for ticker in tqdm(clustered_ticker_list):
        ...
        # symbol.csv 파일 읽기
        ticker_kor = pd.read_csv('./data_kr/symbol.csv')

        # ticker에 해당하는 name을 찾기
        name = ticker_kor[ticker_kor["code"] == ticker]["name"].values[0]

        df_data = pd.read_csv(f"./data_kr/merged/{ticker_str}.csv", index_col=[0])
        df_data.drop(over_50_columns, axis=1, inplace=True, errors="ignore") # 결측치가 0.5가 넘는 것은 제거
        ...
        date = pd.read_csv(f"./data_kr/price/{ticker_str}.csv")
        stock_date = datetime.strptime(df_price.index[0],"%Y-%m-%d")
        print('주식 날짜: ', str(stock_date))
        past_return = []
        #past_start_price = []
        #past_end_price = []
        #past_ks50_end_price = []
        ...
        for i in range(0,len(df_data)-1):
            ...
            print('주식 코드: ' + ticker_str + ', 주식 종목: ' + name + ', 날짜: ' + str(date['날짜'].iloc[i]))
            print('종가: ' + str(end_price) + ', 시가: ' + str(start_price) + ', 주가 변화율: ' +
                  str((end_price - start_price) / start_price))
            print('KOSPI 50 종가: ' + str(ks50_end_price) + ', KOSPI 50 시가: ' + str(ks50_start_price) + ', KOSPI 50 주가 변화율: ' +
                  str((ks50_end_price - ks50_start_price) / ks50_start_price))
            print('시장초과수익률: ' + str((end_price - start_price) / start_price) + ' - ' + str(
                (ks50_end_price - ks50_start_price) / ks50_start_price) + ' = ' + str(relative_return) + '\n')

            # 예제 데이터 (반복문에서 수집된 값들을 리스트에 추가)
            data_list.append([
                ticker_str, name, date['날짜'].iloc[i],
                end_price, start_price, (end_price - start_price) / start_price,
                ks50_end_price, ks50_start_price, (ks50_end_price - ks50_start_price) / ks50_start_price,
                relative_return
            ])
        ...
        df = pd.DataFrame(data_list, columns=[
            '주식 코드', '주식 종목', '날짜',
            '종가', '시가', '주가 변화율',
            'KOSPI 50 종가', 'KOSPI 50 시가', 'KOSPI 50 주가 변화율',
            '시장초과수익률'
        ])

        # CSV 파일 경로 설정 (cluster_index 변수를 사용)
        file_path = f'./data_kr/verification/cluster_2015_RR/cluster_{cluster_index}/{ticker_str}_{name}.csv'
        ...
        print('주식 코드: ' + ticker_str + ', 주식 종목: ' + name + ', 시작 날짜: ' + str(start_date) + ', 종료 날짜: ' + str(end_date))
        print('종가: ' + str(end_price) + ', 시가: ' + str(start_price) + ', 주가 변화율: ' +
              str((end_price - start_price) / start_price))
        print(
            'KOSPI 50 종가: ' + str(ks50_end_price) + ', KOSPI 50 시가: ' + str(ks50_start_price) + ', KOSPI 50 주가 변화율: ' +
            str((ks50_end_price - ks50_start_price) / ks50_start_price))
        print('시장초과수익률: ' + str((end_price - start_price) / start_price) + ' - ' + str(
            (ks50_end_price - ks50_start_price) / ks50_start_price) + ' = ' + str(relative_return) + '\n')

        data_list.append([
            ticker_str, name, start_date, end_date,
            end_price, start_price, (end_price - start_price) / start_price,
            ks50_end_price, ks50_start_price, (ks50_end_price - ks50_start_price) / ks50_start_price,
            relative_return
        ])
        ...
        for i in range(1,len(df_data)):
            ...
            print('시작 년도: ', start_year, ', 종료 년도: ', end_year, '\n시작 분기: ', start_quarter, '종료 분기: ', end_quarter)
            ...
            print('주식 코드: ' + ticker_str + ', 주식 종목: ' + name + ', 시작 날짜: ' + str(start_date) + ', 주식 날짜: ' + str(
                stock_date))
            print('종가: ' + str(end_price) + ', 시가: ' + str(start_price) + ', 주가 변화율: ' +
                  str((end_price - start_price) / start_price))
            print(
                'KOSPI 50 종가: ' + str(ks50_end_price) + ', KOSPI 50 시가: ' + str(
                    ks50_start_price) + ', KOSPI 50 주가 변화율: ' +
                str((ks50_end_price - ks50_start_price) / ks50_start_price))
            print('시장초과수익률: ' + str((end_price - start_price) / start_price) + ' - ' + str(
                (ks50_end_price - ks50_start_price) / ks50_start_price) + ' = ' + str(relative_return) + '\n')

            quater_data_list.append([
                ticker_str, name, start_date, stock_date,
                end_price, start_price, (end_price - start_price) / start_price,
                ks50_end_price, ks50_start_price, (ks50_end_price - ks50_start_price) / ks50_start_price,
                relative_return
            ])
        ...
        # 데이터프레임으로 변환
        df = pd.DataFrame(quater_data_list, columns=[
            '주식 코드', '주식 종목', '시작 날짜', '주식 날짜',
            '종가', '시가', '주가 변화율',
            'KOSPI 50 종가', 'KOSPI 50 시가', 'KOSPI 50 주가 변화율',
            '시장초과수익률'
        ])
        ...
        # CSV 파일 경로 설정 (cluster_index 변수를 사용)
        file_path = f'./data_kr/verification/cluster_quater_RR/cluster_{cluster_index}/{ticker_str}_{name}.csv'
    ...
    # CSV 파일 경로 설정 (cluster_index 변수를 사용)
    file_path = f'./data_kr/verification/cluster_2024Q3_RR/cluster_{cluster_index}/{ticker_str}_{name}.csv'
    ...
    feature_importance.to_csv(f"./data_kr/verification/cluster_feature_importance/cluster_{cluster_index}/feature_importance.csv",encoding='utf-8-sig')
