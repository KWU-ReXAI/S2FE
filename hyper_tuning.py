...
def sweep():
	with wandb.init() as run:
		...
		subprocess.run(f"python train.py --lr_MLP {config.lr_MLP} --lr_anfis {config.lr_anfis} --epochs_MLP {config.epochs_MLP} --epochs_anfis {config.epochs_anfis} --hidden {config.hidden} --Validation", shell=True)
...
sweep_id = wandb.sweep(sweep_config, project="s2fe_hyper_tuning_validation")
