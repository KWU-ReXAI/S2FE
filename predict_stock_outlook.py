...
def predict_market_from_summary(summary: str, stock: str) -> str:
	system_prompt = """
당신은 주어진 기업 소식을 종합적으로 분석하여, 특정 주식 종목의 단기 등락 가능성을 판단하는 정보 분석 전문가입니다.
"""
...
**[분석 작업]**
제공된 콘텐츠에 나타난 정보만을 근거로 "{stock}"의 단기 주가 등락을 판단하세요.

**[출력 지시사항]**
1. ❗️**오직 '+1', '0', '-1' 중 하나의 정수만 출력해야 합니다.**
2. **어떠한 경우에도 설명, 근거, 부가적인 텍스트, 줄바꿈 등 다른 어떤 문자도 포함해서는 안 됩니다.**
...
**[분석 작업]**
제공된 두 가지 콘텐츠(뉴스 기사, 영상 스크립트)에 나타난 정보만 종합적으로 분석하여 "{stock}"의 단기 주가 등락 전망을 판단하세요.
