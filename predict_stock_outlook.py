...
2.  두 콘텐츠를 종합했을 때, 기업 가치나 주가에 미치는 영향이 불분명하거나 상충하는 정보로 인해 판단이 어려울 경우 **'중립'으로 판단하고 점수는 '0'으로 부여**하세요.
3.  **만약 두 콘텐츠의 논조가 명백히 상충할 경우, 'reasoning'에 해당 사실을 반드시 명시**해야 합니다.
4.  출력은 반드시 JSON 객체 {{}}로 이루어져야하며, JSON md markers로 감싸지 마세요.
---	
