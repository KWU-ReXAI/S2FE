...
for K in range(1,6): # 한번만 실행
    ...
    for m in range(2,3): # 20번 실행
        ...
        for phase in tqdm(phase_list): # 각 phase 별 진행상태를 시각적으로 출력
            #model = joblib.load(f"{dir}/final_AGG3_f10_Inter_Cluster/final_AGG3_f10_Inter_Cluster_{m}_{phase}/model.joblib") # 저장된 모델 불러옴
            model = joblib.load(f"{dir}/{args.train_dir}_{K}/train_result_model_{phase}/model.joblib")  # 저장된 모델 불러옴
    ...
    num_stocks_x = [DM.pno2date(i) for i in range(20,60,4)] # 백테스팅 수행 시점의 날짜 리스트
