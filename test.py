...
for K in range(1,args.testNum+1): # 한번만 실행
    ...
    for m in range(2,3): # 20번 실행
        ...
        for phase in tqdm(phase_list): # 각 phase 별 진행상태를 시각적으로 출력
            ...
            cagr, sharpe, mdd, num_stock_tmp,cagr_ks,sharpe_ks,mdd_ks = model.backtest(verbose=True,agg=args.agg,use_all="Sector",inter_n=args.inter_n,withValidation= True, isTest=True, testNum=K, dir=args.test_dir, withLLM=args.LLM, isIntersect=False) # 백테스팅 실행
