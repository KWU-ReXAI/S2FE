...
parser.add_argument('--ensemble',type=str,nargs="?",default="") # 사용할 앙상블 방법
...
dir = f"./result/{args.dir_name}" # 결과를 저장할 디렉토리 생성
if os.path.isdir(dir) == False:
    os.mkdir(dir)


for phase in tqdm(DM.phase_list): # 각 phase 별로 모델 학습 및 평가
    if os.path.isdir(f"{dir}/{args.result_name}_{phase}") == False:
        os.mkdir(f"{dir}/{args.result_name}_{phase}")
    ...
    if args.use_all == "SectorAll" or args.use_all == "All":
        mymodel.trainALLSectorModel()

    if not args.clustering: # clustering을 하지 않는다면
        if args.valid_sector_k != 11: # 섹터 개수를 valid_sector_k에 따라 결정
            if args.use_all == "SectorAll" or args.use_all == "Sector":
                mymodel.trainSectorModels()
                mymodel.sectorModelsValidation()
            else: mymodel.topK_sectors = DM.sector_list
        else: mymodel.topK_sectors = DM.sector_list
    else:
    ...
    mymodel.save_models(f"{dir}/{args.result_name}_{phase}")
