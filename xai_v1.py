...
for sector in ["ALL"] + DM.cluster_list:
    # feature_names 추출
    column_path = f"./data_kr/clustered_data/{sector}/{sector}_T_feature_imp.csv"
...
# 이상치 대체(IQR)
for sec in range(len(sectors_shap)):
    for ph in range(len(sectors_shap[sec])):
        sectors_shap[sec][ph] = np.array([
            np.clip(
                row,
                np.percentile(row, 25) - 1.5 * (np.percentile(row, 75) - np.percentile(row, 25)),
                np.percentile(row, 75) + 1.5 * (np.percentile(row, 75) - np.percentile(row, 25))
            )
            for row in sectors_shap[sec][ph]
        ])
